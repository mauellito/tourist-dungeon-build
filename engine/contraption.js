...
var TD_CONTRAPTION = (function () {
  ...
  var state = { armed: false, armedAt: -1, pulls: 0, placed: false };
  function reset() { state.armed = false; state.armedAt = -1; state.pulls = 0; }   // re-seal the window each life; lever placement persists with the town
  function setPlaced(b) { state.placed = !!b; }         // the town build reports whether a reachable lever actually spawned
  function placed() { return state.placed; }            // the descent seals ONLY when a lever exists (else fail-open — never a soft-lock)
  ...
  return { ARM_TURNS: ARM_TURNS, reset: reset, setPlaced: setPlaced, placed: placed, pull: pull, open: open, remaining: remaining, tick: tick, armed: function () { return state.armed; }, _state: function () { return state; } };
})();
