...
try{
  ...
  // R4 — the dead-end CONNECTOR housing (CSRR/CRMC/Otis/Caspar). Corridor dead-ends are RETIRED under the
  // corridor-first shaper (KEEP: zero unpaid dead-ends), so the stub connector no longer hangs off a corridor
  // tip; the mechanism CODE remains (composeNodeGen2 places one whenever a dead-end tip exists — a hook), and
  // the housing moved to the LATTICE's deadEnd branch sublevels (flagged for CSRR/CRMC/Otis/Caspar). This
  // asserts the connector code path is INTACT (fires on a floor that does have a dead-end tip) without
